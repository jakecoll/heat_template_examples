...
# input flags
tf.app.flags.DEFINE_string("hostname", "", "Must contain 'ps' or 'worker'")
FLAGS = tf.app.flags.FLAGS

parse_error_msg = "'ps' or 'worker ' is not parseable from hostname."
assert '-ps' in FLAGS.hostname or '-worker' in FLAGS.hostname, parse_error_msg

job_name = "ps" if '-ps' in FLAGS.hostname else "worker"
task_index = int(re.findall('\d+', FLAGS.hostname)[-1])
            ...
            for i in range(batch_count):
                ...
                if count % frequency == 0 or i+1 == batch_count:
                    ...
                    print(
                        ...
                        " Cost: %.4f," % cost,
                        " AvgTime: %3.2fms" % float(
                            elapsed_time*1000/frequency))
                    count = 0
